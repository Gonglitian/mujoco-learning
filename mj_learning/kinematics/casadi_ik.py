...
if __name__ == "__main__":
    ...
    import os
    arm.buildFromMJCF(os.path.join(utils.MODEL_ROOT, "trs_so_arm100/so_arm100.xml"))
    theta = np.pi
    tf = np.array([
            [1, 0, 0, 0.1],
    ...
    arm2.buildFromMJCF(os.path.join(utils.MODEL_ROOT, "franka_emika_panda/panda_nohand.xml"))
    theta = np.pi
    tf = np.array([
            [1, 0, 0, 0.1],
            [0, np.cos(theta), -np.sin(theta), 0.2],
