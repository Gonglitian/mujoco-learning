...
class PandaObstacleEnv(gym.Env):
    def __init__(self, visualize: bool = False):
        ...
        import os
        self.model = mujoco.MjModel.from_xml_path(os.path.join(utils.MODEL_ROOT, 'franka_emika_panda/scene.xml'))
...
if __name__ == "__main__":
    ...
    # assets 文件夹在 mujoco-learning 根目录，而 MODEL_ROOT 指向 model 文件夹
    # 所以需要从 MODEL_ROOT 的父目录找到 assets
    ASSETS_ROOT = os.path.join(os.path.dirname(utils.MODEL_ROOT), 'assets')
    MODEL_PATH = os.path.join(ASSETS_ROOT, "model/rl_reach_target_checkpoint/panda_ppo_reach_target_v3")
    RESUME_MODEL_PATH = os.path.join(ASSETS_ROOT, "model/rl_reach_target_checkpoint/panda_ppo_reach_target_v3")
    if TRAIN_MODE:
        train_ppo(
            n_envs=256,                
